...
posterior_PQ = full_posterior_free[, .(P, Q, prob=normalize(p)), key=.(vowel)]
...
ggsave("model_order_posterior_free.png")

# Check top 2, which holds 99% of posterior mass given vowel
top2 = full_posterior_free[order(-p), .SD[1:2, .(MAP, P, Q, p)], key=.(vowel)]
top2[, `prob (%)` := pct(p)]

top2[, sum(`prob (%)`), key=vowel]
top2

posterior_given_vowel = function(...) full_posterior_free[, .(prob = sum(p)), key=c("vowel", c(...))]
...
ggsave("trend_order_posterior_free.png")
